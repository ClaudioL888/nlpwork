...
const sentimentStyles: Record<string, string> = {
  positive: "bg-green-500/20 border-green-500/40",
      ...
      {messages.map((message) => (
        ...
        >
          <div className="flex items-center justify-between text-xs text-slate-300">
            <span className={message.user_id === currentUser ? "text-primary" : "text-slate-400"}>{message.user_id}</span>
            <span>{new Date(message.created_at).toLocaleTimeString()}</span>
          </div>
          <p className="mt-2 text-white">{message.text}</p>
          <div className="mt-2 flex justify-between text-xs text-slate-400">
            <span>Sentiment: {message.sentiment}</span>
            <span>Risk {(message.crisis_probability * 100).toFixed(0)}%</span>
          </div>
        </div>
      ))}
